...
public class UCS {
    ...
    private ZeroSqure currentState;  // تم تعديل نوع الحالة إلى ZeroSqure
    private Set<String> visited = new HashSet<>();
    private List<ZeroSqure> states = new ArrayList<>();
    private int depth = 0;
    private ZeroSqure init;  // تم تعديل نوع الحالة المبدئية إلى ZeroSqure
    ...
    public void ucs() {
      // DeepCopy deepCopy = new DeepCopy();
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (currentState.win()) {  // تم تعديل دالة التحقق من فوز اللعبة
            ...
            if (!visited.contains(boardHash)) {
                ...
                List<ZeroSqure> children = currentState.nextStep();  // تم تعديل دالة nextStep
                for (ZeroSqure child : children) {
                    String childHash = hashBoard(child.getBoard());
                    int costToChild = costMap.get(boardHash) + child.getMoveCost();  // سيتم تنفيذ دالة getMoveCost على ZeroSqure

                    if (!costMap.containsKey(childHash) || costToChild < costMap.get(childHash)) {
                        priorityQueue.add(child);
                        costMap.put(childHash, costToChild);
                        parentMap.put(childHash, currentState);  // تم تعديل استدعاء النسخ
                    }
                }
            }
        }
        ...
        System.out.println("\nThe Cost Of Goal: " + currentState.getMoveCost());  // تم تعديل الاستدعاء
    }

    // Hash board method (assuming it's some unique string hash for a board state)
    private String hashBoard(String[][] board) {  // يتم تعديل الدالة لتأخذ اللوحة من النوع String[][]
        return Arrays.deepToString(board);  // استخدام Arrays.deepToString لإنشاء تمثيل فريد للوحة
    }
}
